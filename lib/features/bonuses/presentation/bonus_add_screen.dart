...
class BonusAddScreen extends ConsumerStatefulWidget {
  const BonusAddScreen({super.key});

  @override
  ConsumerState<BonusAddScreen> createState() => _BonusAddScreenState();
}

class _BonusAddScreenState extends ConsumerState<BonusAddScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      await ref.read(bonusServiceProvider).createBonus(payload);
      ref.invalidate(bonusListProvider);
      
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Bonus added successfully')));
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('Bonus Type', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              DropdownButtonFormField<String>(
                initialValue: _type,
                items: ['Performance', 'Festival', 'Year-end', 'Other']
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: _saving ? const CircularProgressIndicator(color: Colors.white) : const Text('Add Bonus'),
                ),
              ),
            ],
          ),
